use anyhow::Result;
use std::io::{Read, Write};
use log::*;
use stderrlog::new;

static S: [u8; 256] = [
    252, 238, 221, 17, 207, 110, 49, 22, 251, 196, 250, 218, 35, 197, 4, 77,
    233, 119, 240, 219, 147, 46, 153, 186, 23, 54, 241, 187, 20, 205, 95, 193,
    249, 24, 101, 90, 226, 92, 239, 33, 129, 28, 60, 66, 139, 1, 142, 79,
    5, 132, 2, 174, 227, 106, 143, 160, 6, 11, 237, 152, 127, 212, 211, 31,
    235, 52, 44, 81, 234, 200, 72, 171, 242, 42, 104, 162, 253, 58, 206, 204,
    181, 112, 14, 86, 8, 12, 118, 18, 191, 114, 19, 71, 156, 183, 93, 135,
    21, 161, 150, 41, 16, 123, 154, 199, 243, 145, 120, 111, 157, 158, 178, 177,
    50, 117, 25, 61, 255, 53, 138, 126, 109, 84, 198, 128, 195, 189, 13, 87,
    223, 245, 36, 169, 62, 168, 67, 201, 215, 121, 214, 246, 124, 34, 185, 3,
    224, 15, 236, 222, 122, 148, 176, 188, 220, 232, 40, 80, 78, 51, 10, 74,
    167, 151, 96, 115, 30, 0, 98, 68, 26, 184, 56, 130, 100, 159, 38, 65,
    173, 69, 70, 146, 39, 94, 85, 47, 140, 163, 165, 125, 105, 213, 149, 59,
    7, 88, 179, 64, 134, 172, 29, 247, 48, 55, 107, 228, 136, 217, 231, 137,
    225, 27, 131, 73, 76, 63, 248, 254, 141, 83, 170, 144, 202, 216, 133, 97,
    32, 113, 103, 164, 45, 43, 9, 91, 203, 155, 37, 208, 190, 229, 108, 82,
    89, 166, 116, 210, 230, 244, 180, 192, 209, 102, 175, 194, 57, 75, 99, 182
];

static INV_S: [u8; 256] = [
    
];

static L: [u8; 16] = [1, 148, 32, 133, 16, 194, 192, 1, 251, 1, 192, 194, 16, 133, 32, 148];

static POWS: [u8; 255] = [
    1, 2, 4, 8, 16, 32, 64, 128, 195, 69, 138, 215, 109, 218, 119, 238,
    31, 62, 124, 248, 51, 102, 204, 91, 182, 175, 157, 249, 49, 98, 196, 75,
    150, 239, 29, 58, 116, 232, 19, 38, 76, 152, 243, 37, 74, 148, 235, 21,
    42, 84, 168, 147, 229, 9, 18, 36, 72, 144, 227, 5, 10, 20, 40, 80,
    160, 131, 197, 73, 146, 231, 13, 26, 52, 104, 208, 99, 198, 79, 158, 255,
    61, 122, 244, 43, 86, 172, 155, 245, 41, 82, 164, 139, 213, 105, 210, 103,
    206, 95, 190, 191, 189, 185, 177, 161, 129, 193, 65, 130, 199, 77, 154, 247,
    45, 90, 180, 171, 149, 233, 17, 34, 68, 136, 211, 101, 202, 87, 174, 159,
    253, 57, 114, 228, 11, 22, 44, 88, 176, 163, 133, 201, 81, 162, 135, 205,
    89, 178, 167, 141, 217, 113, 226, 7, 14, 28, 56, 112, 224, 3, 6, 12,
    24, 48, 96, 192, 67, 134, 207, 93, 186, 183, 173, 153, 241, 33, 66, 132,
    203, 85, 170, 151, 237, 25, 50, 100, 200, 83, 166, 143, 221, 121, 242, 39,
    78, 156, 251, 53, 106, 212, 107, 214, 111, 222, 127, 254, 63, 126, 252, 59,
    118, 236, 27, 54, 108, 216, 115, 230, 15, 30, 60, 120, 240, 35, 70, 140,
    219, 117, 234, 23, 46, 92, 184, 179, 165, 137, 209, 97, 194, 71, 142, 223,
    125, 250, 55, 110, 220, 123, 246, 47, 94, 188, 187, 181, 169, 145, 225
];

static IN_POWS: [u8; 255] = [
    0, 1, 157, 2, 59, 158, 151, 3, 53, 60, 132, 159, 70, 152, 216, 4,
    118, 54, 38, 61, 47, 133, 227, 160, 181, 71, 210, 153, 34, 217, 16, 5,
    173, 119, 221, 55, 43, 39, 191, 62, 88, 48, 83, 134, 112, 228, 247, 161,
    28, 182, 20, 72, 195, 211, 242, 154, 129, 35, 207, 218, 80, 17, 204, 6,
    106, 174, 164, 120, 9, 222, 237, 56, 67, 44, 31, 40, 109, 192, 77, 63,
    140, 89, 185, 49, 177, 84, 125, 135, 144, 113, 23, 229, 167, 248, 97, 162,
    235, 29, 75, 183, 123, 21, 95, 73, 93, 196, 198, 212, 12, 243, 200, 155,
    149, 130, 214, 36, 225, 208, 14, 219, 189, 81, 245, 18, 240, 205, 202, 7,
    104, 107, 65, 175, 138, 165, 142, 121, 233, 10, 91, 223, 147, 238, 187, 57,
    253, 68, 51, 45, 116, 32, 179, 41, 171, 110, 86, 193, 26, 78, 127, 64,
    103, 141, 137, 90, 232, 186, 146, 50, 252, 178, 115, 85, 170, 126, 25, 136,
    102, 145, 231, 114, 251, 24, 169, 230, 101, 168, 250, 249, 100, 98, 99, 163,
    105, 236, 8, 30, 66, 76, 108, 184, 139, 124, 176, 22, 143, 96, 166, 74,
    234, 94, 122, 197, 92, 199, 11, 213, 148, 13, 224, 244, 188, 201, 239, 156,
    254, 150, 58, 131, 52, 215, 69, 37, 117, 226, 46, 209, 180, 15, 33, 220,
    172, 190, 42, 82, 87, 246, 111, 19, 27, 241, 194, 206, 128, 203, 79
];

fn multiply(a: u8, b: u8) -> u8 {
    if a == 0 || b == 0 {
        return 0;
    }
    let inda = IN_POWS[a as usize - 1];
    let indb = IN_POWS[b as usize - 1];
    let mut indmul = inda as usize + indb as usize;
    if indmul >= 255 {
        indmul -= 255;
    }
    POWS[indmul]
}

pub fn linear_transform(block: &mut [u8; 16]) {
    for _ in 0..16 {
        let mut cur = 0u8;
        for first_ind in 0..16 {
            cur ^= multiply(L[15 - first_ind], block[first_ind]);
        }
        block[15] = cur;
        block.rotate_right(1);
    }
}

pub fn get_round_constants() -> [[u8; 16]; 32] {
    let mut constants = [[0u8; 16]; 32];
    for i in 0..32 {
        constants[i][15] = i as u8 + 1;
        linear_transform(&mut constants[i]);
    }
    constants
}

pub fn generate_keys(key: &[u8; 32]) {
    let mut round_keys: [[u8; 16]; 10] = [[0u8; 16]; 10];
    round_keys[0].copy_from_slice(&key[16..]);
    round_keys[1].copy_from_slice(&key[0..16]);

    let round_constants = get_round_constants();
    for i in 1..2 {
        let mut left = round_keys[2 * (i - 1)];
        let mut right = round_keys[2 * (i - 1) + 1];

        for ft_iter in 0..8 {
            let mut new_right = [0u8; 16];
            for ind in 0..16 {
                new_right[ind] = S[(left[ind] ^ round_constants[(i - 1) * 8 + ft_iter][ind]) as usize];
            }
            linear_transform(&mut new_right);
            for ind in 0..16 {
                new_right[ind] ^= right[ind];
            }
            left = right;
            right = new_right;
            info!("{:#2x?}, {:#2x?}", left, right);
        }
        round_keys[i * 2] = left;
        round_keys[i * 2 + 1] = right;
    }
    // info!("{:#2x?}", round_keys);
}

pub fn encode<R: Read, W: Write>(mut input: R, mut output: W) -> Result<()> {
    let mut key = [
        0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10, 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef,
        0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77];
    generate_keys(&key);
    Ok(())
}

pub fn decode<R: Read, W: Write>(input: R, mut output: W) -> Result<()> {
    Ok(())
}